...
resource "openstack_compute_instance_v2" "hq3_worker" {
  ...
  depends_on = [
    #openstack_compute_instance_v2.hq3_server,
...
resource "null_resource" "worker_configure" {
  ...
  count       = var.n_workers
  depends_on = [  openstack_compute_instance_v2.hq3_worker ]
  ...
  connection {
    ...
    user             = "ubuntu"
    private_key      = file("../z03.pem")
    host             = openstack_compute_instance_v2.hq3_worker[count.index].network[0].fixed_ip_v4
    bastion_user     = "centos"
    bastion_host     = var.bastion_ip
  }
}
...
# resource "openstack_blockstorage_volume_v3" "hq3_worker" {
#   count       = var.n_workers
...
# resource "openstack_compute_volume_attach_v2" "hq3_worker_vattach" {
#   count       = var.n_workers
#   instance_id = openstack_compute_instance_v2.hq3_worker[count.index].id
#   volume_id  = openstack_blockstorage_volume_v3.hq3_worker[count.index].id
#   #device = "/dev/vdb"
# }
...
#   triggers =  {
#     disksize = openstack_blockstorage_volume_v3.hq3_worker[count.index].size
#   }
...
#       <<EOT
#       DEVICE=${openstack_compute_volume_attach_v2.hq3_worker_vattach[count.index].device}
#       PART="$DEVICE"1
